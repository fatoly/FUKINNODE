...
//Формирует пакет на отправку в сеть узлов и шлет на нужную шину
void NodeKernel::CreatePacGlobal(std::vector<int> PacGlobal)
{
}
//Формирует пакет на отправку в сеть УСУ и шлет на нужную шину
...
//Возвращает сколько шин на эмулируемом узле
...
void NodeKernel::Events(std::vector<int> OnePac)
{
	...
	//Обработанный пихаем обратно в OnePac
